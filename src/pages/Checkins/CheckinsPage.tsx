...
const PAGE_SIZE = 8
const STATUS_FILTERS = ['TODOS', 'AUTORIZADO', 'BLOQUEADO'] as const
...
export function CheckinsPage() {
  ...
  const filteredCheckins = useMemo(() => {
    ...
    return checkins.filter((checkin) => {
      const alunoNome = checkin.alunoNome ?? '-'
      const statusLabel = checkin.permitido ? 'AUTORIZADO' : 'BLOQUEADO'
      const matchesStatus = statusFilter === 'TODOS' || statusLabel === statusFilter
      const matchesQuery = normalizedQuery
        ? normalizeText(
            [checkin.id, alunoNome, statusLabel, checkin.motivo ?? '', checkin.dataHora].join(' '),
  ...
  const visibleCheckins = filteredCheckins.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE)
  ...
  return (
    <>
      <PageHeader
        ...
        description="Historico de entradas autorizadas ou bloqueadas."
      />
      ...
      <section className="content-panel">
        ...
        {!isLoading && !errorMessage && checkins.length > 0 && filteredCheckins.length === 0 && (
          <StateMessage title="Nenhum check-in corresponde aos filtros" description="Ajuste a busca ou o filtro de status." />
        )}
          ...
          <>
            <DataTable headers={['ID', 'Aluno', 'Data e hora', 'Status', 'Motivo']}>
              {visibleCheckins.map((checkin) => (
                <tr key={checkin.id}>
                  <td>{checkin.id}</td>
                  <td>{checkin.alunoNome ?? '-'}</td>
                  <td>{formatDate(checkin.dataHora)}</td>
                  <td>
                    <StatusBadge status={checkin.permitido ? 'AUTORIZADO' : 'BLOQUEADO'} />
                  </td>
                  <td>{checkin.motivo ?? '-'}</td>
                </tr>
              ))}
            </DataTable>
            <TablePagination
...
function getStatusFilterFromSearch(search: string): StatusFilter {
